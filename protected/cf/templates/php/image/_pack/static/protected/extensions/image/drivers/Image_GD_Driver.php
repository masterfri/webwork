...
class Image_GD_Driver extends Image_Driver {
	...
	public function process($image, $actions, $dir, $file, $render = FALSE)
	{
		...
		$quality = isset($actions['quality']) ? $actions['quality'] : null;
    ...
    protected function watermark($args)
	{
        ...
        $overlay = $create($watermark->__get('file'));
		...
		imagealphablending($this->tmp_image, TRUE);
        $imgHeight = imagesy($this->tmp_image);
        $imgWidth = imagesx($this->tmp_image);
        
        //exit;
		if ($status = imagecopyresampled($this->tmp_image, $overlay, $offset_x, $offset_y, 0, 0, $dst_w, $dst_h, $width, $height)){
